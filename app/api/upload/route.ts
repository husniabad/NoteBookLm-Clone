          ...
          if (pages.length > 0) {
            ...
            if (allChunks.length > 0) {
              ...
              const insertPromises = allChunks.map((chunk, i) => 
                sql`INSERT INTO documents (content, embedding, type, session_id, source_file, is_standalone_file) VALUES (${chunk}, ${JSON.stringify(embeddings[i].embedding.values)}, 'text', ${sessionId}, ${file.name}, FALSE)`
              );
...
Be thorough - this is the only analysis for this page. Format: [Page ${i + 1}] [COMPLETE] full description.`;
          ...
          if (descriptions.length > 0) {
            ...
            const insertPromises = descriptions.map((desc, i) => 
              sql`INSERT INTO documents (content, embedding, type, session_id, source_file, is_standalone_file) VALUES (${desc}, ${JSON.stringify(embeddings[i].embedding.values)}, 'text', ${sessionId}, ${file.name}, FALSE)`
            );
            await Promise.all(insertPromises);
          }
        ...
        await sql`INSERT INTO documents (content, embedding, type, session_id, source_file, is_standalone_file) VALUES (${richDescription}, ${JSON.stringify(embedding)}, 'image', ${sessionId}, ${file.name}, TRUE)`;
      }
        ...
        await Promise.all(textChunks.map(async (chunk) => {
          ...
          const embedding = embeddingResult.embedding.values;
          await sql`INSERT INTO documents (content, embedding, type, session_id, source_file, is_standalone_file) VALUES (${chunk}, ${JSON.stringify(embedding)}, 'text', ${sessionId}, ${file.name}, TRUE)`;
        }));
      }
    }));

    return NextResponse.json({ success: true, message: 'File processing complete.' });
